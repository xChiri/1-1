...
public class gamecontroller : MonoBehaviour {
	...
	private int l , c;
	public int[,] privatematrix = new int[5, 5];
	...
	void RunAd()
	{
		if (requestinterstitialvar == false) {
			//RequestInterstitial();
			requestinterstitialvar = true;
		}
	...
	void CheckIfAllSpheresAreAtTheirPlace()
	{
		for (int i = 0; i <= 4; i++)
			for (int j = 0; j <= 4; j++)
				if (privatematrix [i, j] != values [i, j])
